...
void loop()
{
    ...
    {                          // Wenn der Taster aktuell gedrÃ¼ckt wird, ...
    ...
    if (digitalRead(PIN_BUTTON) == HIGH && aButtonPressed == true)
    {                           // Wenn der Taster nicht gedrÃ¼ckt wird, aber zuvor gedrÃ¼ckt wurde ...
        aButtonPressed = false; // vergesse, dass der Taster gedrÃ¼ckt wude.

        if (aLedEnabled == true)
        {                               // Wenn die LED gerade leuchtet ...
